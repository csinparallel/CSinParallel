...
int main(int argc, char* argv[]) {
	...
	double startT, stopT; //wall clock elapsed time
...
// defaults
	...
	getArguments(argc, argv, &maxSpins, &numThreads, &useConstantSeed);
	...
	startT = omp_get_wtime();     // start the timer
...
/**************************** Do Simulations ***/		
	while (numSpins < maxSpins) {
		...
		numWins = getNumWins(numSpins, seed, numThreads);	
...
/**************************** Finish Up ********/
	stopT= omp_get_wtime();		//stop our timer & show elapsed time
	cout<<"\nElapsed wall clock time: "<< (double)(stopT-startT)<<endl<<endl;
...
//always bet 'red' & count wins
	...
	shared(numSpins, myBet, min, max, seed, numThreads) \
	private(spin, nextRandVal, tid) \
	reduction(+:wins)
	{
		trng::yarn2 rand;
		rand.seed(seed);
		...
		if (numThreads > 1) {
			// thread will get substream tid from numThread 
			// separate substreams- this is leapfrog
			rand.split((unsigned)numThreads, tid);  
		}
	...
	}  // end parallel block of forked threads
